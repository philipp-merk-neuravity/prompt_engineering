...
from .template_functions import get_messages_for_refinement, get_messages_for_code_generation, get_messages_for_syntax_correction, get_messages_for_test_generation, get_messages_for_prompt_preprocessing, get_messages_for_self_reflection
...
async def evaluate_syntax_for_code_solution(code_solution: str, function_signature: str) -> str:
    ...
    while not is_syntax_correct:
        messages = get_messages_for_syntax_correction(function_signature, code_solution, error_message)
...
async def gen_reflection(function_implementation: str, unit_test_results: str, model: str) -> str:
    messages = await get_messages_for_self_reflection(function_implementation, unit_test_results)
    return await get_completion(messages, model=model)

async def gen_refined_function(function_signature: str, function_implementation: str, unit_test_results: str, reflection: str, model: str) -> str:
    messages = await get_messages_for_refinement(function_signature, function_implementation, unit_test_results, reflection)
    refined_function, prompt_tokens, completion_tokens, duration = await get_completion(messages, model=model)
    code_block = await parse_code_block(refined_function)
